...
# alpha is from 0 - 1 It adjusts the transparency 1 is total black, 0 is nothing/blank
```
...
Based on the results from the ANOVA F TEST we see that P value is extreme small (Pr(>F) <2e-16 ) meaning that we have extremely strong evidence against the null hypothesis and reject it, that the population mean weight is the same between all fourteen rodent species. Thus, this means that at least one rodent species has a different population mean weight.
